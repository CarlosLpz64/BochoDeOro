...
export class LowerComponent implements OnInit {
  ...
  constructor(private _snackBar: MatSnackBar, private meta: Meta, private title: Title, private fb: FormBuilder) {
    this.title.setTitle('Obtener el valor más bajo de una lista');
    this.meta.updateTag({ name: 'title', content: 'Obtener el valor más bajo de una lista' });
    this.meta.updateTag({ name: 'description', content: 'Obtén el valor menor de una lista de números' });
  ...
  doAction() {
    ...
    const result = Math.min(...tempNumbers)
    if (isNaN(result)) {
      this._snackBar.open('Error mostrar valor. Verifica el texto de entrada o el separador.', '', {duration: 3000});
      this.result = "0"
      return
